...
class UpdateRecipeUseCaseTest {
    ...
    private val useCase = UpdateRecipeUseCase(repository)

    @Test
    fun `should invoke repository and return a Recipe updated when is invoked`() {
        val recipe = TestHelper.getRecipe()
        whenever(repository.updateRecipe(recipe)).thenReturn(Single.just(recipe))
    ...
    fun `should invoke repository and return error when is invoked and get error from Repository`() {
        val recipe = TestHelper.getRecipe()
        whenever(repository.updateRecipe(recipe)).thenReturn(Single.error(ApiException.InternalServerError))
